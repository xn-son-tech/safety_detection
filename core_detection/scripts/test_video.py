...
def draw(frame: np.ndarray, decisions: list[PersonDecision], detections: list[Detection]) -> np.ndarray:
    ...
    for d in detections:
        ...
        cv2.putText(out, f"{d.class_name} {d.confidence:.2f}", (x1, max(20, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run unified helmet pipeline on a video")
    ...
    print(f"[test_video] Model Path: {MODEL_PATH} ({model_size} bytes)")
    ...
    out_dir = PROJECT_ROOT / "test_results" / "videos"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"{source_path.stem}_result.mp4"
    ...
    try:
        while True:
            ...
            output_frame = draw(enhanced, decisions, filtered)
